...
with tab5:
    ...
    rup_pdswsql = con.execute(f"SELECT * FROM df_sw_umumkan WHERE namasatker = '{opd}'").df()
    rup_pdswsql_tampil = con.execute(f"SELECT idrup AS KODE_RUP, namapaket AS NAMA_PAKET, tipe_swakelola AS TIPE_SWAKELOLA, ppk AS PPK, volume AS VOLUME, jumlahpagu AS NILAI_PAGU FROM rup_pdswsql ").df()
